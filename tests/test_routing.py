...
def test_raamsdonksveer_roundabout_routing():
    ...
    for row in gdf_raw.itertuples():
        ...
        coords = list(geom.coords)
        way_nodes_map[osm_id] = [get_node_key(Point(c)) for c in coords]
        
        if osm_id in blocked_ids or highway in ['motorway', 'motorway_link', 'trunk', 'trunk_link'] or '"motorroad"=>"yes"' in other_tags:
            continue
    ...
    # Maasdijk entry coordinate and Keizersveer destination coordinate
    n_start = (51.70816, 4.88532)
    n_end = (51.70933, 4.88879)

    start_node = min(G.nodes, key=lambda n: (n[0] - n_start[0])**2 + (n[1] - n_start[1])**2)
    end_node = min(G.nodes, key=lambda n: (n[0] - n_end[0])**2 + (n[1] - n_end[1])**2)

    path = nx.shortest_path(G, source=start_node, target=end_node, weight='weight')
    used_ways = [G[path[i]][path[i+1]]['osm_id'] for i in range(len(path) - 1)]

    # 1. Path must be found safely
    assert len(path) > 0
    # 2. Path MUST NOT route onto A27 on-ramp stub (Way 7264139)
    assert '7264139' not in used_ways
