...
- In [routing.xml](file:///home/tomaso/projects/brombrom/config/routing.xml), forbidden highway access rules (`microcar=no`, `motorway`, `motorway_link`, `trunk`, `trunk_link`, `motorroad=yes`) use `select value="0"` in `<way attribute="access">`, paired with `select value="0.0"` in `<way attribute="priority">` and `select value="1"` in `<way attribute="speed">`, instead of `value="-1"` in access.
- **Maneuver Engine Exit Counting**: Hard-dropping edges (`value="-1"`) strips forbidden ramps from OsmAnd's network graph, causing OsmAnd's maneuver engine to skip counting physical roundabout exits (e.g. calling the 2nd physical exit the "1st exit"). Setting `value="0"` in access alongside `priority="0.0"` retains the road in the topological network so OsmAnd accurately counts all physical exits at roundabouts (*"Neem de 2e afslag naar Keizersveer"*), while assigning infinite routing penalty (`weight = Distance / (Speed * Priority) = ∞`) so Dijkstra / A* route planning NEVER routes microcars onto restricted highways.
