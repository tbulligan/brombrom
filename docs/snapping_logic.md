...
- **Intersection Warning Bonus (Conditional)**: If no major candidate road (`priority <= 2`) matches the NDW sign's road name (e.g. when the sign is posted on a minor side-street approach like *Het Elferman* at an N-road T-junction), the name penalty is reduced to `0.0` for any high-priority **non-link** road (`priority <= 2` and not ending in `_link`) within 50 meters whose bearing matches the sign's bearing within 30 degrees. This allows intersection entry and pre-warning signs on side streets to correctly snap to the main restricted highway instead of getting trapped on minor residential streets.
