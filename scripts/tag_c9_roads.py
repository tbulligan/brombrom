...
def main():
    if os.path.exists("BromBrom-NL.osm.pbf"):
        print("BromBrom-NL.osm.pbf already exists. Skipping.")
        return
    ...
    with osmium.SimpleWriter("BromBrom-NL.osm.pbf", overwrite=True) as writer:
    ...
    print(f"✓ BromBrom-NL.osm.pbf created ({len(forbidden_ways)} C9 ways processed)")
